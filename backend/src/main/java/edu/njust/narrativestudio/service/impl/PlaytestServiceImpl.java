...
public class PlaytestServiceImpl implements PlaytestService {
    ...
    public PlaytestServiceImpl(PlaytestSessionMapper sessions,PlaytestStepMapper steps,StoryNodeMapper nodes,
            ...
            RuleEngine engine,ObjectMapper json) {
    ...
    @Transactional
    public SessionView start(Long u,Long p) { guard.member(u,p); return startInternal(u,p); }
    private SessionView startInternal(Long u,Long p) {
        List<StoryNode> starts=nodes.selectList(new LambdaQueryWrapper<StoryNode>().eq(StoryNode::getProjectId,p).eq(StoryNode::getIsStart,true));
        if(starts.size()!=1) throw invalidState("剧情必须有且只有一个起点");
        StoryNode start=starts.getFirst(); validateNodeType(start);
        Map<String,String> initial=engine.initialState(catalog.variables(p));
        ...
        sessions.insert(s);
        PlaytestStep step=step(s,0,start.getId(),null,initial,initial); steps.insert(step);
        return view(s,start,step);
    }
    public SessionView get(Long u,Long p,Long id) {
        access.requireMember(u,p); PlaytestSession s=owned(u,p,id); return view(s,node(p,s.getCurrentNodeId()),latest(id));
    }
    @Transactional
    public SessionView advance(Long u,Long p,Long id,Long choiceId,int expectedStepNo) {
        ...
        StoryNode current=node(p,s.getCurrentNodeId());
        if("ENDING".equals(current.getNodeType())) throw invalidState("当前节点已是结局");
        StoryChoice c=choices.selectById(choiceId);
        ...
        List<StateVariable> vars=catalog.variables(p);
        Map<String,String> before=decode(previous.getStateAfter());
        if(!engine.available(vars,catalog.conditions(choiceId),before)) throw invalidState("当前状态不满足选择条件");
        StoryNode target=node(p,c.getTargetNodeId());
        Map<String,String> after=engine.apply(vars,catalog.effects(choiceId),before);
    ...
    public SessionView restart(Long u,Long p,Long id) {
        ...
        return startInternal(u,p); // Fresh identity; old steps are retained for replay.
    }
    @Transactional
    public SessionView stop(Long u,Long p,Long id) {
        ...
        return view(s,node(p,s.getCurrentNodeId()),latest(id));
    }
    public Page<SessionView> list(Long u,Long p,int page,int size) {
        ...
        return new Page<>(rows.stream().map(s->view(s,node(p,s.getCurrentNodeId()),latest(s.getId()))).toList(),page,size,total,(total+size-1)/size);
    }
    public Page<StepView> steps(Long u,Long p,Long id,int page,int size) {
        ...
        return new Page<>(rows.stream().map(s->new StepView(s.getId(),s.getStepNo(),s.getNodeId(),s.getChoiceId(),
                decode(s.getStateBefore()),decode(s.getStateAfter()),s.getCreatedAt())).toList(),page,size,total,(total+size-1)/size);
    }
    private SessionView view(PlaytestSession s,StoryNode current,PlaytestStep latest) {
        ...
        if("RUNNING".equals(s.getStatus()) && !"ENDING".equals(current.getNodeType())) {
            List<StateVariable> vars=catalog.variables(s.getProjectId());
            for(StoryChoice c:choices.selectList(new LambdaQueryWrapper<StoryChoice>().eq(StoryChoice::getProjectId,s.getProjectId())
                ...
                if(engine.available(vars,catalog.conditions(c.getId()),state))
