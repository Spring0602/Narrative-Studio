-- Test-only mirror of database/schema.sql. No production schema changes.
-- H2 stores snapshot JSON in CLOB: MySQL JSON binding must still be verified with Navicat/MySQL.
...
CREATE TABLE IF NOT EXISTS playtest_step (
  ...
  state_before CLOB NOT NULL,
  state_after CLOB NOT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
